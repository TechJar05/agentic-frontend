...
const AdminDashboard = () => {
  const { user, token } = useAuth(); // Access token directly
  const adminId = user?.id;

  console.log("Auth Data:", { user, token, adminId }); // Debug auth values

  const { mds, loading, error, changeMDStatus } = useMDs(adminId, token);
  const [searchTerm, setSearchTerm] = useState("");
  const [activeFilter, setActiveFilter] = useState("all");
  ...
  const handleApprove = (id) => {
    changeMDStatus(id, "approved");
  };

  const handleReject = (id) => {
    changeMDStatus(id, "rejected");
  };

  const handleDelete = (id) => {
    changeMDStatus(id, "deleted");
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white shadow-sm rounded-lg overflow-hidden">
          <div className="px-4 py-5 sm:px-6 border-b border-gray-200">
            <h2 className="text-lg font-medium text-gray-600">MD List</h2>
            <p className="mt-1 text-sm text-gray-500">
              {filteredMDs.length} MD{filteredMDs.length !== 1 && "s"} found
            </p>
          </div>
